...
export interface ISupportingFriend {
  id: string;
  name: string;
  profilePicture: string;
  patientId: string;
...
export interface AnalyticsData {
  assessments: AnalyticsDataItem[];
  moods: AnalyticsDataItem[];
  activities: AnalyticsDataItem[];
}
...
export interface SelectedDataPoints {
  point1: string;
  point2: string;
}
...
export interface AssessmentData {
  ...
  lastScore: number;
  riskLevel: string;
  riskCategory: { min: number; max: number; label: string }[];
  questions: Array<{
    id: number;
    text: string;
...
export interface INotesData {
  notes: INote[];
  healthCare: IHealthcareProvider;
  id: string;
}
...
export interface IUser {
  ...
  areaOfFocuses?: string;
  is_verified?: boolean;
}

// for now we will use it later we will change it
...
interface User {
  ...
  healthcareProviders: DocumentReference[]; // Ref to Users
  ...
  notificationSettings: UserNotificationSettings; // Only applicable to role == "Healthcare Provider"
  patients: DocumentReference[] | null; // Ref to Users

  isOnboardingComplete?: boolean; // added for health care providers because health care will not do on boarding while signing up they will do at first login and we will set it to true
...
interface AssessmentSessionQAs {
